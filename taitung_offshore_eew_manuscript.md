...
The same geometry also undermines two quality indicators that are routine for inland sources. Azimuthal gap — the largest azimuthal sector without a station — is widely used as a location-quality flag: a small gap means stations surround the source. For an offshore epicentre a correct location necessarily leaves a large gap, often greater than 180°, because every station lies on land; a small gap appears only if the epicentre is wrongly placed inside the network. A rule that withholds public alerts when the gap is large would therefore block valid offshore warnings. Magnitude error is a similarly poor promotion gate. The first stations to trigger occupy a narrow landward sector of the focal sphere, so the early $Pd$ magnitude samples one radiation lobe rather than an azimuthal average. An initial overestimate — or an underestimate, if that sector is nodal — is then an expected geometric accident, not a diagnosis that the solution is unfit to release. A more informative test is whether the stations used for magnitude already span more than one azimuthal sector, and whether independent parallel streams agree on the location.

This paper closes that gap using the complete operational record of a moderate offshore earthquake. We document the eBEAR response report by report, covering all six parallel processing streams and all three public dissemination channels; we decompose the first-alert magnitude bias into a location-distance term, an azimuthal-sampling term and an intrinsic relation term; and we test which physical mechanism controls the azimuthal term, using the focal mechanism of the event. We then ask whether the result generalises, by recomputing identical metrics for three further Taiwan earthquakes spanning offshore and near-coast geometries, and we evaluate magnitude error and azimuthal gap as promotion gates against alternatives based on magnitude-station azimuthal coverage and cross-stream location consistency. Section 2 describes the system and data, Section 3 the evaluation metrics and analysis protocol, Sections 4 to 6 the results, and Sections 7 and 8 the discussion and conclusions.
...
| Medium | Use median or trimmed-mean station $M_{Pd}$ rather than the mean of a few stations | With three stations, a single 0.5-unit outlier shifts the event magnitude by 0.17 |
